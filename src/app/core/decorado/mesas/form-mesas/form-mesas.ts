...
import { MesaModel } from '../../../../models/salon/mesa/mesa.model';
...
@Component({
  ...
  templateUrl: './form-mesas.html',
  styleUrl: './form-mesas.css',
})
export class FormMesas {

  idsalonxdxd=input<string>();
  salir=output<boolean>();
  ///
  ///
  data = input<MesaModel>();
  ///
  ///
  ...
  guardar(){
    ...
    if(this.editando()){
      //editando
      this.dbFire.edit(PATHFireBase.Mesas,this.data()?.id??'',data_);
      this.onSalir();
    }
    else{
      //nuevo
      const uid=uuidv4();
      this.dbFire.add(PATHFireBase.Mesas,uid,data_);
      this.onSalir();
    }
    ///////////////////////////////////
    //////////////////////////////////
