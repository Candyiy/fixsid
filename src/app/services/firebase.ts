...
import { Database, get, onValue, ref, remove, set, update, query, orderByChild, startAt, endAt} from '@angular/fire/database';
...
export class Firebase {
  ...
  async adddos(path: string, id: string, data: any) {
    const dbref = ref(this.db, path+'/'+id);
    return set(dbref, data);
  }
  async editdos(path: string, id: string, data: any) {
      const dbref = ref(this.db, `${path}/${id}`);
  ...
  obtenerTodosUsuarios(/*path:PATHFireBase*/): Observable<any[]> {
    const usuariosRef = ref(this.db, 'users');
    return new Observable(observer => {
      onValue(usuariosRef, snapshot => {
        const data = snapshot.val();
        
        const arr = Object.entries(data).map(([key, value]: any) => ({ id: key, ...value }));
        observer.next(arr);
      });
    });
  }
